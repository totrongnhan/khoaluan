...
if (isset($_GET['req'])) {
    switch ($_GET['req']) {
        ...
        case "export":
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('G1', "MOTA");
            $objPHPExcel->getActiveSheet()->SetCellValue('E1', "PHANQUYEN");
            $objPHPExcel->getActiveSheet()->SetCellValue('F1', "PHANNHOM");
            ...
            foreach ($taikhoan__Get_All as $item) {
                // $objPHPExcel->getActiveSheet()->SetCellValue('A' . $row_hd, "" . $item->id_Sinh_Vien);
                $objPHPExcel->getActiveSheet()->SetCellValue('A' . $row_hd, "" . $item->tentahoan);
                $objPHPExcel->getActiveSheet()->SetCellValue('B' . $row_hd, "" . $item->email);
                $objPHPExcel->getActiveSheet()->SetCellValue('C' . $row_hd, "" . $item->matkhau);
                $objPHPExcel->getActiveSheet()->SetCellValue('D' . $row_hd, "" . $item->mota);
                $objPHPExcel->getActiveSheet()->SetCellValue('E' . $row_hd, "" . $item->id_phanhom);
                $objPHPExcel->getActiveSheet()->SetCellValue('F' . $row_hd, "" . $item->id_phanquyen);
                $objPHPExcel->getActiveSheet()->SetCellValue('G' . $row_hd, "" . $item->id_nguoidung);
