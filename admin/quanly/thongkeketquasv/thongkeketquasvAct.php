...
if (isset($_GET["req"])) {
    switch ($_GET["req"]) {
        case "add":
            ...
            $phieukhaosat__Get_By_Id_phieudot = $phieukhaosat->phieukhaosat__Get_By_Id_phieudot($id_dot ,$id_tenkhaosat);
            if (count($phieukhaosat__Get_By_Id_phieudot) > 0) {
                foreach ($phieukhaosat__Get_By_Id_phieudot as $item) {
                    ...
                    $status .= $thongkeketquasv->thongkeketquasv_Add($id_tenkhaosat, $id_dot, $id_phieu, $id_lophoc, $id_sinhvien, $ma_sinhvien, $tensinhvien, $kohl, $ithl, $khahl, $hl, $rathl, $per_kohl, $per_ithl, $per_khahl, $per_hl, $per_rathl);
                }
            }

            if ($status != 0) {
                header("Location: ../../index.php?req=thongkeketquasv&id_tenkhaosat=$id_tenkhaosat&id_dot=$id_dot&id_lophoc=$id_lophoc&status=success");
            } else {
                header("Location: ../../index.php?req=thongkeketquasv&id_tenkhaosat=$id_tenkhaosat&id_dot=$id_dot&id_lophoc=$id_lophoc&status=fail");
            }
            break;
        ...
        case "delete_all":
            ...
            $status = $thongkeketquasv->thongkeketquasv__Delete_All($id_tenkhaosat);
            if ($status) {
                header("Location: ../../index.php?req=thongkeketquasv&status=success");
            } else {
                header("Location: ../../index.php?req=thongkeketquasv&status=fail");
